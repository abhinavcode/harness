...
func (r *LocalRegistry) FetchArtifact(ctx context.Context, info pkg.MavenArtifactInfo, serveFile bool) (
	...
	dbImage, err2 := r.DBStore.ImageDao.GetByName(ctx, info.RegistryID, name, types.SoftDeleteFilterExcludeDeleted)
	if err2 != nil {
		return processError(err2)
	}

	if !utils.IsMetadataFile(info.FileName) || info.Version != "" {
		_, err2 = r.DBStore.ArtifactDao.GetByName(ctx, dbImage.ID, info.Version, types.SoftDeleteFilterExcludeDeleted)
		if err2 != nil {
