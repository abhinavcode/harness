...
) (*GeneralSettingsSpace, error) {
	space, err := c.getSpaceCheckAccess(ctx, session, spaceRef, enum.PermissionRepoEdit)
	if err != nil {
		return nil, err
	}

	// read old settings values
	old := GetDefaultGeneralSettingsSpace()
	oldMappings := GetGeneralSettingsMappingsSpace(old)
	err = c.settings.SpaceMap(ctx, space.ID, oldMappings...)
	if err != nil {
		return nil, fmt.Errorf("failed to map settings (old): %w", err)
	}

	err = c.settings.SpaceSetMany(ctx, space.ID, GetGeneralSettingsAsKeyValuesSpace(in)...)
	if err != nil {
		return nil, fmt.Errorf("failed to set settings: %w", err)
	}

	// read all settings and return complete config
	out := GetDefaultGeneralSettingsSpace()
	mappings := GetGeneralSettingsMappingsSpace(out)
	err = c.settings.SpaceMap(ctx, space.ID, mappings...)
	if err != nil {
