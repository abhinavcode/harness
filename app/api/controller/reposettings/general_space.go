...
func GetDefaultGeneralSettingsSpace() *GeneralSettingsSpace {
	return &GeneralSettingsSpace{
		DefaultBranch: ptr.String(settings.DefaultBranch),
	}
}

func GetGeneralSettingsMappingsSpace(s *GeneralSettingsSpace) []settings.SettingHandler {
	return []settings.SettingHandler{
		settings.Mapping(settings.DefaultBranchKey, s.DefaultBranch),
	}
}

func GetGeneralSettingsAsKeyValuesSpace(s *GeneralSettingsSpace) []settings.KeyValue {
	kvs := make([]settings.KeyValue, 0, 1)

	if s.DefaultBranch != nil {
		kvs = append(kvs, settings.KeyValue{
			Key:   settings.DefaultBranchKey,
