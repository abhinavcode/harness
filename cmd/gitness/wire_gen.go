...
func initSystem(ctx context.Context, config *types.Config) (*server.System, error) {
	...
	manifestService := docker.ManifestServiceProvider(registryRepository, manifestRepository, blobRepository, mediaTypesRepository, manifestReferenceRepository, tagRepository, imageRepository, artifactRepository, layerRepository, gcService, transactor, accessorTx, eventReporter, spaceFinder, ociImageIndexMappingRepository, artifactReporter, provider, auditService)
	...
	apiHandler := router.APIHandlerProvider(registryRepository, upstreamProxyConfigRepository, fileManager, tagRepository, manifestRepository, cleanupPolicyRepository, imageRepository, storageDriver, spaceFinder, transactor, accessorTx, authenticator, provider, authorizer, auditService, artifactRepository, webhooksRepository, webhooksExecutionRepository, service3, spacePathStore, artifactReporter, downloadStatRepository, config, registryBlobRepository, registryFinder, asyncprocessingReporter, registryHelper, spaceController, quarantineArtifactRepository, spaceStore, packageWrapper, cacheService, finder)
	packageTagRepository := database2.ProvidePackageTagDao(db)
	localBase := base.LocalBaseProvider(registryRepository, registryFinder, fileManager, transactor, accessorTx, imageRepository, artifactRepository, nodesRepository, packageTagRepository, authorizer, spaceFinder)
	...
	mavenLocalRegistry := maven.LocalRegistryProvider(localBase, mavenDBStore, transactor, accessorTx, fileManager, spaceFinder)
	...
	genericController := generic.ControllerProvider(spaceStore, authorizer, fileManager, genericDBStore, transactor, accessorTx, spaceFinder, genericLocalRegistry, proxy, finder, dependencyFirewallChecker)
